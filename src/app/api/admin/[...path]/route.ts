...
async function proxyAdminRequest(request: NextRequest, path: string[]): Promise<NextResponse> {
  ...
  const backendResponse = await fetch(`${getBackendBaseUrl()}/${path.join("/")}${search}`, {
...
export async function GET(request: NextRequest, context: { params: { path: string[] } }) {
  return proxyAdminRequest(request, context.params.path);
}
